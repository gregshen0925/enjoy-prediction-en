...
const NavigationTab = ({ activeNum, changeActiveNum }: Props) => {
    ...
    return (
        <div className="bg-gray-200 max-h-[4.4rem] px-6 rounded-t-xl">
            <ul className="flex relative">
                <span
                    className={`bg-blue-500 duration-200 ${Menus[activeNum].dis} border-4 border-black h-16 w-16 absolute -top-5 rounded-full`}
                >
                    <span
                ...
                {Menus.map((menu, i) => (
                    <li key={i} className="w-16">
                        <Link href={menu.router}>
                            ...
                            >
                                <span
                                    className={`text-xl cursor-pointer px-[22px] duration-300 ${i === activeNum && "-mt-5 text-black z-30"
                                        }`}
                                >
                                    {menu.icon}
                                </span>
                                <span
                                    className={` ${activeNum === i
